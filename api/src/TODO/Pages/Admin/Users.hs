...
renderUserList users = html_ $ do
  head_ $ do
    ...
    link_
      [rel_ "stylesheet", href_ "https://cdn.jsdelivr.net/npm/tailwindcss@2.2.19/dist/tailwind.min.css"]
  body_ [class_ "bg-gray-100 min-h-screen flex items-center justify-center"] $ do
    div_ [class_ "w-full max-w-4xl bg-white shadow-md rounded p-6"] $ do
      div_ [class_ "flex justify-between items-center mb-4"] $ do
        ...
        a_
          [href_ "/admin/users/new", class_ "bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600"]
          "＋ 新規ユーザー登録"
  ...
  body_ [class_ "bg-gray-100 min-h-screen flex items-center justify-center"] $ do
    div_ [class_ "w-full max-w-md bg-white shadow-md rounded p-6"] $ do
      ...
      form_ [method_ "post", action_ "/admin/users/new"] $ do
        -- 名前入力欄
        div_ [class_ "mb-4"] $ do
          label_ [for_ "userName", class_ "block text-gray-700 font-bold mb-2"] "名前"
          input_
            [ type_ "text"
            , id_ "userName"
            , name_ "userName"
            , required_ ""
            , class_ "w-full border rounded px-3 py-2"
            ]

        -- パスワード入力欄
        div_ [class_ "mb-6"] $ do
          label_ [for_ "userPassword", class_ "block text-gray-700 font-bold mb-2"] "パスワード"
          input_
            [ type_ "password"
            , id_ "userPassword"
            , name_ "userPassword"
